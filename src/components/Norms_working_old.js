...
const Norms = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [selectedOption, filterLogicData, selectedOptions]);
...
//    alert("hi")
  ...
  const renderTable = () => {
    ...
    return (
      ...
      <Row>
      <Col sm={12}>
  <table id="normsTable" className="table-container table table-bordered table-striped" style={{ width: "80%", borderCollapse: 'collapse', border: '1px groove black', marginLeft: '0' }}>
    <thead>
      <tr>
        <th>Percentile</th>
    ...
    <thead>
      ...
      <tr>
        <td style={{ textAlign: 'left', border: '1px groove black' }}><strong>Number of projects</strong></td>
        <td colSpan={headers.length} style={{ textAlign: 'left', border: '1px groove black ' }}>{noprojects}</td>
      </tr>
    </thead>
  </table>
  </Col>
  ...
  <Col sm={8}> {renderFiltersDropdown()}</Col>
  ...
  {isValid && (
    ...
    />
      ...
      <Col sm={3}>
    <div className='dropsection' style={{ border: '1px solid #ced4da', borderRadius: '5px', padding: '20px', backgroundColor:'#0D3A4B', paddingTop:'5px'}}>
    <Button  variant="danger" onClick={handleReset} disabled={!Object.values(selectedOptions).some(options => options.length > 0)} style={{marginBottom:'5px', marginLeft:'90px'}} >Reset</Button>
      <Row>
