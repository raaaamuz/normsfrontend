...
const Norms = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        const fetchDropdownOptions = async (question) => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/norms/percentiles/', {
        ...
        try {
            ...
            const response = await axios.get(`http://127.0.0.1:8000/norms/percentiles/`, {
                params: {
                    normsVar: selectedOption,
                    filter_name: filterLogicData,
                    filters: selectedOptionsString,
                    question: Object.keys(selectedOptions).join(',')
                },
                headers: {
                    Authorization: `Token ${localStorage.getItem('Token')}`,
                },
  ...
  const handleOptionDeselect = (option, question) => {
    setSelectedOptions(prevOptions => ({
      ...prevOptions,
  ...
  const getSelectedOptionsString = (question) => {
    if (selectedOptions[question]) {
      return selectedOptions[question].join(' AND ');
    }
    return '';
  };
  ...
  //   try {
  //     const selectedOptionsString = Object.keys(selectedOptions).map(question => {
  //       const options = selectedOptions[question].join(' (AND) ');
  //       return options ? `${question}: ${options}` : ''; 
  //     }).filter(Boolean).join('; ');
  
  //     await axios.post('http://127.0.0.1:8000/filter/filter-names/', {
  //       filters: selectedOptionsString
  //     },
  //   );
  ...
  //   fetchData();
  // }, [selectedOption,filterLogicData]);
...
//    alert("hi")
  ...
  const renderTable = () => {
    ...
    const filteredPercentiles = percentiles.filter(percentile => (percentile !== 'Mean Value' && percentile !== 'Samples considered'&& percentile !== 'Number of products'));
